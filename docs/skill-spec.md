...
> 2026-09-25 决定：Skill 生成只由用户在对话中明确指定一次已完成工作后触发；Agent 不主动判断工作是否值得保存。本文以下“自动沉淀 / 后台复盘”章节是 2026-09-22 的旧提案，不再是目标行为，实施前须按此决定修订。当前实现仍生成待审核草稿，见 `status.md`。
...
| 自动沉淀（已取消） | 2026-09-22 曾提议后台复盘近期轨迹 | 不实施 |
...
3. **明确学习**：用户指定一次已完成工作并要求总结 → Agent 读取该工作，生成简洁标题与可复用做法；没有这一用户要求时不创建 Skill。
