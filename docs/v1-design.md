...
各域的接口字段与语义单独成文，位于 `docs/contracts/`：`mail.md`、`calendar.md`（已实现），`personal-kb.md`（Phase 1 与 Phase 2 已实现：资料的保存、读取、更新、历史版本，以及分节检索与按引用读取原文；回答不展示来源；Phase 3–5 为约定），`skills.md`（Memory 已实现，Skills 为约定）。记忆与个人资料库的产品行为分别见 `memory-spec.md` 与 `kb-spec.md`。本文不重复契约字段。
...
- **Context** 每轮装配常驻材料：长期记忆文件、进行中的目标与资料主题目录，各有容量上限；历史对话、资料正文、邮件与日程不自动注入。
...
| Context | 每轮装配基础提示与常驻材料（`USER.md`、`MEMORY.md`、`GOALS.md`、资料主题目录），控制各块容量；不预先检索资料或历史 | `server/agent/context.py` |
| Agent Loop | 通过 Qoder Agent SDK 调用模型和工具，加载已生效 Skills，按目标决定下一步 | `server/agent/` |
| Memory | 保存用户背景、偏好、约定与进行中的目标，每轮判断与后台回顾负责写入；不记录出处 | `server/memory/` |
...
│   │   ├── context.py        # 每轮系统提示、常驻材料（记忆、目标、资料主题目录）与技能名单组装
...
模型每轮可见范围由注册时声明的副作用与轮次类型共同决定：触发轮允许只读，以及注册时声明“触发轮可用”的本地写（只有资料库的新建与修改，Phase 3 起）；用户对话轮允许只读、本地写与直接外部写；执行结果回传轮允许只读与本地写；只在用户对话轮可见的本地写（资料删除、移动、恢复）不出现在其他轮次；永不暴露的外部写在任何一轮都不出现。因此纯触发轮不会在用户未参与时产生待确认内容、写入外部服务、沉淀 Skill 草稿或删除资料；它能做的本地写只是保存或更新资料，结果有提示、有版本、可恢复。结果回传轮的输入同样来自系统而非用户亲自开口，也拿不到直接外部写，邮件与资料内容中的指令无法据此驱动外部写入。
...
| Personal KB | 保存、索引、检索、更新、删除与恢复 Markdown 资料，维护主题页，导入外部笔记，归档任务关键信息与结果；内部引用带文件路径、原文位置和内容版本，只用于读取原文，不作为出处 | `contracts/personal-kb.md` |
...
个人知识库保存 Markdown 原件与派生索引，以文件为准。索引是数据目录内的派生 SQLite 文件，不进 Git：按二级标题分节后建立 FTS5 全文索引，应用写入在同一把资料库锁内只替换受影响文件的条目，缺失、损坏或与 `kb/` 目录的 Git tree 不一致时整体重建；Phase 3 起检索与写入前先把用户在文件系统里的改动提交为版本，再更新索引。内部引用带文件路径、原文位置与内容版本（commit），只用于读取原文与历史版本；回答依据由模型先检索、再读取原文得到，回答不展示来源，程序不记录回答来源，资料与记忆都不记录出处。

长期状态与知识的组织参考 OpenHuman 的 Memory Tree 与 SuperContext（2026-09-16 查阅），借鉴思路而不照搬实现，完整取舍见 `kb-spec.md` 第 12 节：

- **常驻与按需分开**：每轮只注入小容量的常驻材料（记忆三文件与资料主题目录），其余按需检索，常驻材料总量由各块容量上限控制。
...
任务结束后，Agent 通过 KB 工具归档关键信息及逐项结果，并区分原始内容和模型总结（Phase 4）。
...
按 `v1-spec.md` 第 5 节的两个验收场景逐步交付完整链路。本节状态随实施更新；阶段 2 已完成代码接入，阶段 3 进行中：Calendar、长期 Memory 最小闭环与 Personal KB Phase 1、Phase 2（Markdown 文件 + Git 版本的保存/读取/更新，SQLite FTS5 分节索引、按引用读取原文；回答不展示来源）已接入；资料的自动跟随、删除与恢复、管理界面、主题页与导入，进行中的目标、Memory 管理页面与历史检索尚未实现；自动化测试使用外部边界替身，真实账号验收尚未完成。
...
已实现：Web、Gateway、Agent 装配、Gmail 域、Calendar 域、长期 Memory 文件与每轮专用记忆判断（主 Agent 不再持有记忆工具）、后台记忆回顾、Personal KB Phase 1 与 Phase 2（`kb_list`/`kb_save`/`kb_read`/`kb_update`/`kb_history`/`kb_search`，资料为 `kb/` 下的 Markdown 文件，由与 Memory 同一个数据目录本地 Git 仓库做版本；`kb-index.sqlite3` 是数据目录内的派生 FTS5 索引，按二级标题分节、写入后增量替换、以 `kb/` 的 Git tree 标识判断是否需要重建；引用带完整 commit 与行号区间，可按引用读回该版本原文，回答不展示来源；模块在 `server/tools/personal_kb/`，经 `server/storage/datarepo.py` 与 Memory 共享进程内锁与 `.gitignore`；新增 `pyyaml` 依赖）、Session Store 与 Confirmation。SQLite schema 为 12（资料身份写在 frontmatter、版本走 Git；schema 11 曾新增回答来源表，schema 12 随撤销来源展示将其删除）。未实现：文件改动自动跟随（Phase 3）、资料管理界面（Phase 4）、Skills、Memory 管理页面与历史检索、认证与 HTTPS 远程访问。
...
界面只呈现接口能支撑的内容：邮件草稿卡可编辑并确认，日程不渲染卡片，创建结果只在对话文字里汇报、记录留在操作与执行数据中；资料库回答不展示来源，资料浏览器、编辑入口与全局搜索框对应的接口尚未提供，暂不渲染。
...
- 生效 Skill 名单恒为空，Skills 尚无代码；Personal KB 已实现 Phase 1 与 Phase 2（资料的保存、读取、更新、历史版本，分节检索与按引用读取；回答不展示来源），文件改动自动跟随、资料管理界面尚未实现；Memory 已有两个长期记忆文件、每轮专用记忆判断、后台记忆回顾、逐轮加载与本地 Git 历史，管理页面、历史检索和恢复尚未实现。
- 资料仍保留出处相关代码：frontmatter 的 `source` 字段、`kb_save`/`kb_update` 的 `source` 参数（缺省时以任务为来源）与 `kb_search` 的 `source_kind` 筛选；按 `personal-kb.md` 移除。
- 新邮件触发轮当前只允许只读工具，尚不能按 `kb-spec.md` 主动保存资料；需在工具注册中增加“触发轮可用”的声明（Phase 3）。
- 进行中的目标（`GOALS.md`）、资料主题目录、后台主题整理与外部笔记导入尚无代码；常驻上下文目前只有 `USER.md` 与 `MEMORY.md`。
