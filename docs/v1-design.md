...
│   ├── memory/               # Memory
│   │   ├── service.py        # 两份记忆文档的读写、按行锚点编辑与容量检查
...
回顾是一次性 SDK 会话：临时 MCP 端点只暴露 `review_registry` 的 `memory_edit`，输入是窗口内对话文本与带锚点的当前记忆快照；网关按记录到的工具结果返回，实际生效的替换、删除与跨分区移动渲染为“整理记忆：…”提示，与回顾结束状态同一事务写入时间线（挂在窗口内最后一个调用上），再经 SSE 推送；新增不提示。登记时冻结窗口上下界（`from_rowid`/`through_rowid`），执行期新消息不进窗口。
