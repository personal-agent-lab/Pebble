...
### Calendar 创建链路（2026-09-13）

本次按用户缩减后的范围交付，详细接口、配置、异常与验收见 [日程创建说明](calendar-create.md)。
`server/tools/calendar/` 包含 CalDAV 客户端、本地预览存储及三个模型工具；创建与核实由
Confirmation 服务端入口调用。固定账号/日历绑定在预览中，修改配置不重定向已保存操作。
SQLite schema 4 扩展操作状态，新增日程预览版本表及独立结果 JSON，保留邮件结果字段及行为。
Gateway 通过调用输入中的 operation_id 定位结果，允许待核实日程的成功核实作为一次新的
结果通知回到原会话。模型仍由 Qoder SDK 执行，不引入自有 Agent 循环。
Web 复用任务页和确认页，按操作类型渲染日程字段与结果；确认绑定已展示预览的版本。
本次不实现原目标中的日程查询、冲突检查、资料归档或其他跨工具能力。
