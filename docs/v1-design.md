...
│   ├── api/            # HTTP/SSE 路由与错误映射（认证随阶段 6 加入）
...
工具由应用进程自己的 MCP 端点按轮次暴露（每轮一个一次性路径）。注册表有三个：前台对话、后台记忆回顾（`memory_edit`）与每轮记忆判断（另加 `memory_ask`），后两个只在各自的一次性会话中出现。
...
- 时间线按顺序保存：连续文字合并为一项，草稿工具成功时在当前位置插入卡片。卡片只存 `operation_id`，读取时物化最新版本与执行状态；直接编辑与 Agent 修改都只新增版本，不移动卡片、不新增第二张。SSE 只传实时变化，断开不取消执行，结束后前端重读时间线对账。
...
两者都是一次性 SDK 会话，MCP 端点只挂各自的记忆工具，不接续任务会话。程序按记录到的真实工具结果渲染提示（`memory/notices.py`），以 `notice` 写入时间线并经 SSE 推送，模型自述不作为依据。

- 判断：每个用户消息轮与主回答并行启动，输入为本条消息、近期对话与带锚点的当前记忆；无状态，不写 `agent_runs`、不占运行槽，失败只记日志，不影响回答。
...
- **Skills**：用户自建即生效；Agent 从使用记录总结的草稿存放在 SDK 发现目录之外，在对话中提示审阅。批准绑定内容版本，内容再变化即停止加载、回到草稿。加载器只提供已批准且版本一致的内容，经 SDK 的 `skills` 配置限定范围，恢复会话时同步名单。批准 Skill 不改变外部写的授权规则。
...
设计只固定安全要求，传输方案（反向隧道、反向代理加域名或私有网络）在部署时选定并写入 `README.md`：

- 只暴露 HTTPS。
- 单用户认证：未认证请求不能读取任务、时间线、草稿、资料、Memory 与 Skill，也不能提交消息或确认。
- 确认与执行接口校验身份与请求来源，身份来自已认证会话而非请求体。
- 令牌可轮换，失效立即拒绝，登出使令牌失效；SSE 与轮询使用同一认证。
...
- 步骤说明：工具执行前按注册时声明的 `activity_renderer` 生成一句说明，经 `activity` 事件转发；不写时间线，只在内存保留最后一步供刷新读取；调用失败时附在错误说明末尾。
...
- 路由：`/tasks`、`/tasks/:taskId`、`/search`、`/kb`、`/kb/doc`、`/kb/new`、`/memory`。断点 900px，以上为侧栏布局，以下为底部 tab；设计 token 见 `src/styles/tokens.css`。
...
- 草稿卡有未保存修改时不能确认，确认绑定卡片当前展示的版本；日程不渲染卡片；Skill 入口置灰。
