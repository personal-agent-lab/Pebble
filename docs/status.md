...
- Skills 实现仍采用随机 id、四态草稿审批和前台 `skill_propose`；2026-09-25 起只响应用户明确指定的一次已完成工作，不再要求三个任务，也不主动判断是否值得保存。`skill-spec.md`/`contracts/skill.md` 中 2026-09-22 提出的后台自动沉淀已被这一用户决定覆盖，相关规格和重构步骤须先修订；其余目录、附件与持久轨迹设计尚未实现。
