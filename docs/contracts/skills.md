...
| 用户纠正 | 任务时间线中的记忆提示（“已更新记忆”，具体改动见记忆页面） | 同类纠正是否重复出现 |
...
- `text` 去掉两端空白后不能为空，可以多行；任何一行以锚点前缀（`xxxx| `）开头即报错，程序不替模型删除。`end_anchor` 必须与 `anchor` 同一分区且不在它之前；`to` 必须是另一个分区。
...
成功输出 `changed`（整次调用是否改动了记忆）、`applied[]`（按顺序：`action`、`target`，移动另有 `from`、`to`，`removed`/`added` 为程序实际删除与写入的整行，`changed`，未改动时 `reason` 为 `exists` 或 `same`），以及 `memory`：改动过的分区各自带锚点的最新 `content` 与 `usage`。几处修改互相抵消、分区没有变化时每处都记为未改动。`memory_ask` 输入一句确认问题，不做任何写入，问题由程序作为提示展示给用户。

每轮判断的写入结果与追问、后台回顾实际生效的替换、删除与移动，由程序按 `applied[]` 渲染成用户可见的提示（见 `memory-spec.md` §5.3），模型自述不作为事实来源；回顾的新增与失败不提示。判断中 `memory_edit` 失败的，只有它是这次判断最后一次 `memory_edit` 调用时才提示失败。回顾提示以 `notice` 写入任务时间线，挂在回顾窗口内最后一个调用上，并经 SSE 推送。
...
Memory 字段校验（含页面接口的未知分区）、锚点不存在或修改范围冲突返回 `invalid_memory` 并附 `errors[]`（HTTP 422），模型工具调用出错时另附 `memory`（两个分区带锚点的最新内容与用量）；容量超限返回 `memory_full`，附 `target`、`used`、`limit` 与 `memory`（HTTP 422）；文件不可读写返回 `memory_store_unavailable`（HTTP 503）。失败不保留文件的半完成修改。Skills 字段校验仍使用 `SkillValidationError`，附 `errors[]`。
