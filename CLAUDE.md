...
对话优先的个人助手 Agent：常驻 Python 服务，用户随时对话，由单个 Agent 按目标组合工具（Gmail、iCloud Calendar、个人知识库、Memory/Skills），程序保证确认、持久化与去重。新邮件是首版触发源，不是唯一入口。

## 文档

- `docs/v1-spec.md`：需求范围、产品行为、验收标准。内容冲突时以此为准。
- `docs/v1-design.md`：组件与代码结构（§2）、交付阶段（§7）、验证要求（§9）、当前实现与已知偏差（§10）。
- `docs/contracts/`：按域的接口字段与语义。`mail.md` 已实现；`calendar.md`、`personal-kb.md`、`skills.md` 是约定，尚未实现。
