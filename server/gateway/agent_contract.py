...
class AgentEvent(TypedDict, total=False):
    """Agent 事件；type 取 session、text、notice、draft_saved、source、done、error。"""
...
def checked_event(event: object) -> AgentEvent:
    ...
    kind = event["type"]
    if kind not in {"session", "text", "notice", "draft_saved", "source", "done", "error"}:
    ...
    if kind == "source":
        ...
        if (
            ...
            or not isinstance(ref.get("commit"), str)
        ):
            raise AgentProtocolError(f"source 事件缺少引用：{event!r}")
