...
工具由装配方通过 `server/agent/toolset.py` 绑定好依赖后传入，本模块不查找全局工具状态。
"""
...
class QoderGateway:
    ...
    async def _stream(
        ...
        try:
            ...
            options = build_options(
                self.tools, task_id, sdk_session_id, execution_result, draft_events, allow_drafts
            )
    ...
    def stream_message(self, *, task_id, sdk_session_id, message):
        return self._stream(task_id, message, sdk_session_id)
    ...
    def stream_new_mail(self, *, task_id, sdk_session_id, source_message_id, thread_id):
        ...
        return self._stream(task_id, message, sdk_session_id, allow_drafts=False)
    ...
    async def stream_execution_result(
        ...
        *,
        ...
        payload = {
            ...
            "version": version,
            "result": {
                key: result[key] for key in ("status", "message_id", "reason") if key in result
            },
        }
