...
const draft: Extract<TimelineItem, { kind: "mail_draft" }> = {
  ...
  draft: {
    ...
    attachments: [{
      file_id: "file-1", filename: "proof.pdf", mime_type: "application/pdf", size: 12, sha256: "abc",
    }],
  },
...
test("按持久化顺序渲染 Agent 文字、完整邮件卡和后续文字", () => {
  const items: TimelineItem[] = [
    { item_id: "text-1", kind: "text", role: "assistant", run_id: "run-1", text: "前置说明", attachments: [], created_at: "2026-09-14T00:00:00Z" },
    draft,
    { item_id: "text-2", kind: "text", role: "assistant", run_id: "run-1", text: "后置说明", attachments: [], created_at: "2026-09-14T00:00:01Z" },
  ];
  ...
  expect(screen.getByText("proof.pdf · 12 B")).toBeTruthy();
});
