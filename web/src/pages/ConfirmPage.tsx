 ...
 * 单个操作的编辑与确认。
 *
 * 确认绑定页面当前展示的内容：内容变化后旧确认自动失效（后端按版本判定并返回冲突）。
 * 版本号只用于请求，不出现在界面上。每个操作分别确认、分别执行，页面不提供批量确认。
 */
    ...
    <section className="op-card" data-component="MailDraftCard">
      ...
      <div className="confirm-body">
        ...
        <aside className="side">
          ...
          <div className="effect">
            ...
            发送上述完整内容。执行内容与你确认时看到的逐字段一致，确认输入不含正文。
          </div>
          {view.execution?.confirmation != null && (
            <div className="meta-list">
              <h5>确认记录</h5>
              <div className="meta-row">
                <span className="id">已确认</span>
                <span>{shortTime(view.execution.confirmation.confirmed_at)}</span>
              </div>
              {result !== null && (
                <div className="meta-row">
                  <span className="id">
      ...
      {failure !== null && (
        <div style={{ padding: "0 18px 14px" }}>
          ...
          >
            {failure.code === "version_conflict" ? (
              "你操作时依据的不是最新内容，草稿已被更新。内容变化后旧确认不再适用，请查看上方最新内容后重新确认。这次操作没有发出任何邮件。"
            ) : failure.code === "invalid_draft" ? (
              "请按上方提示修改后重新保存。校验通过不表示已保存、已确认或已发送。"
            ) : (
              failure.message
            )}
          </Notice>
      ...
      <div className="page">
        ...
        <div className="page-head">
          <div>
            <h2>草稿编辑与确认</h2>
            <div className="sub">
              关闭或刷新页面后，以下内容仍可继续编辑与确认；执行内容将与你确认的逐字段一致。
            </div>
          </div>
