...
export default function SkillPicker({value, onChange, disabled = false}: {value: Selection; onChange: (s: Selection) => void; disabled?: boolean}) {
  const [items, setItems] = useState<Skill[]>([]), [open, setOpen] = useState(false), [query, setQuery] = useState(""), [error, setError] = useState("");
  ...
  return <div className="skill-picker">
    ...
    <button type="button" className="btn-secondary" disabled={disabled} aria-expanded={open} onClick={() => setOpen(!open)}>添加 Skill</button>
    {open && <div className="skill-panel"><input aria-label="搜索 Skill" placeholder="搜索名称或描述" value={query} onChange={e => setQuery(e.target.value)} />
