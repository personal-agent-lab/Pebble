...
当前状态：已增加 Agent 日程预览、网页编辑与确认创建链路（配置见下文）；A/B 分支已合并，任务、草稿版本、确认执行、Gateway、网页与 Qoder CN/Gmail
生产装配已连接，SQLite schema 为 4。支持新邮件增量检测、任务历史、SSE、Agent 草稿修改、
...
界面只呈现接口能支撑的内容：设计稿中的来源引用面板、搜索框对应的接口尚未提供，暂不渲染；日程预览卡已按本次创建范围接通。
...
见 [日程创建说明](docs/calendar-create.md)。配置 `.env.example` 中的 iCloud 账号、App 专用密码文件、
固定目标日历 URL 和默认时区，重启服务后在现有任务对话里说：

> 帮我安排 2026 年 10 月 12 日上午 10 点到 11 点的项目会议，时区 Australia/Adelaide，地点会议室。

Agent 只保存预览；在操作卡或编辑页点击「确认创建」后才写入。未保存修改时不能确认。
支持单次定时事件，不查询日程、不检查冲突、不发送邀请，也不自动发送邮件。
未配置日历时，邮件功能维持原行为；生产入口仍使用现有 Qoder CN/Gmail 装配。

验收：149 项后端测试通过（含 22 项日历测试），Ruff 与前端类型检查、构建通过。
桌面 1280×860、手机 375×812 浏览器视口通过：准备、编辑、未保存阻止确认、刷新恢复、
最终内容一致及重复确认不重复创建。浏览器、HTTP、SQLite 为真实执行，模型与日历写入为替身。
真实 iCloud 创建尚未验收，须配置账号并由用户确认测试日程后单独验证。
